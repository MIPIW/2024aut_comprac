...
data_raw = str(Path().resolve().parent()) + os.sep + "data/data_raw"
data_processed = str(Path().resolve().parent()) + os.sep + "data/data_raw_processed"
...
def convert_raw_data_to_training_data(str_file_name, training_file_name, valid_file_name, test_file_name, meta_file_name, value_cols):
    ...
    df = pd.Series(lst1d).sample(frac = 1, random_state = 42) # random state는 나중에 추가함. 본 데이터셋 해당 안 됨.
    df_test = pd.Series(lst1d_test).sample(frac = 1, random_state = 42) # random state는 나중에 추가함. 보 ㄴ데이터셋 해당 안 됨
